# Page节点向量嵌入生成工具
# 用于将Page节点中的文本属性转换为向量嵌入并存储到Neo4j数据库
...
def process_page_embeddings():
    ...
    try:
        ...
        with driver.session(database=target_db) as session:
            ...
            # 执行 Cypher 查询：获取所有 Page 节点及其指定属性
            result = session.run("""
            MATCH (n:Page)
            RETURN id(n) AS id,
                   n.name AS name,
            ...
            print(f"📊 找到 {total_records} 个Page节点，开始生成向量嵌入...")

            if total_records == 0:
                print("⚠️  未找到任何Page节点，请检查数据库中是否存在Page标签的节点")
                return
            ...
            for i, record in enumerate(records, 1):
                try:
                    ...
                    # 添加名称信息
                    if record.get('name'):
                        text_parts.append(f"页面名称: {record['name']}")
                    if record.get('name_cn'):
                        text_parts.append(f"中文页面名称: {record['name_cn']}")
...
# === 批量处理多个属性作为单独向量 ===
def process_page_embeddings_separate():
    """
