...
When reading [`exportSettings`](/docs/plugins/api/node-properties/#exportsettings), always check the `format` before reading other properties.
...
The [`exportAsync`](/docs/plugins/api/properties/nodes-exportasync/) function supports an additional [`ExportSettingsSVGString`](#exportsettingssvgstring) type for exporting a node to an SVG string.
...
This returns a JSON object in the shape of the response of the <https://api.figma.com/v1/files> endpoint. For more information read about the API [here](/docs/rest-api/file-endpoints/#get-files-endpoint).
