...
This section provides a brief overview of how your plugin accesses the document of a Figma, FigJam, or Figma Slides file and some of the APIs you are most likely to use. Check out the [reference](/docs/plugins/api/api-reference/) to see the full extent of the Figma Plugin API.
...
**DocumentNode**
The [DocumentNode](/docs/plugins/api/DocumentNode/) is the root of the node tree in Figma, FigJam, and Figma Slides files. Each Figma file contains a single instance of a DocumentNode. A DocumentNode contains one or more PageNodes.

**PageNode**
[PageNodes](/docs/plugins/api/PageNode/) are the immediate descendants of the DocumentNode. A Figma design or FigJam file contains one or more PageNodes (one for each page in the document). Slide decks in Figma Slides only have a single PageNode, accessed using `figma.currentPage`.
